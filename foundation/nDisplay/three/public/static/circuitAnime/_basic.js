...
class CircuitAnime {
    ...
    play() {
        ...
        function solveEquation___readyCallback() {
                    ...
                    // self.smoothMoveCameraPosition(facingCoordinate.x, facingCoordinate.y, facingCoordinate.z ,function() {
                        ...
                        function equationReveal(theList, theList___idx) {
                            ...
                            self.pauseUntilCallback(()=>{
                                ...
                                //until check audio is in Sync before continuing
            ...
            //get the audioFile for solveEquations before we start the anime
            self.getAudioUrls_solveEquations(
                function(subtitles, list_tuple_word_location_length_elapsedTime, filename, pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime){
                    animeSelf.subtitles_solvingSteps=subtitles; 
                    animeSelf.subtitleIdx__dict_startTime_endTime_solvingSteps=subtitleIdx__dict_startTime_endTime; 
                    animeSelf.filename_solvingSteps = filename;
                    animeSelf.gotAudio_solveEquations = true;
                    pauseTimings = pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime.entries().sort((a, b)=>a[0]-b[0]).map(([pauseIdx, d]) => {return d['endTime']})//list of timings to pause
                    self.audioPlayer = self.getWavFile(function(audioBuffer){
                        self.makeWavPlayer(audioBuffer, pauseTimings, 
                ...
                self.solvingSteps,
                animeSelf.language_introduction_solvingSteps, 
                animeSelf.language_conclusion_solvingSteps
            );
            self.play(function() {
                ...
                return function() {
                    ...
                    function equationDisplay(list_equationNetworkInfoDict, list_equationNetworkInfoDict___idx) {
                        if (list_equationNetworkInfoDict___idx >= list_equationNetworkInfoDict.length) {
                            ...
                            animeSelf.updateCurrentPositionInAnimation();
                            animeSelf.pauseUntilVideoAndAudioInSync();
                            return;
                        }
...
// console.log('equationIdx: ', list_equationNetworkInfoDict___idx, graphInfoD['equation'], '>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
                    ...
                    self.pauseUntilCallback(
                        ...
                        function() {//to check if we have obtained the audiofilepaths(if the tickets are completed)
                            console.log('checking if flag: animeSelf.gotAudio_findEquations is true: ', animeSelf.gotAudio_findEquations)
                            console.log('checking if flag: animeSelf.gotAudio_solveEquations is true: ', animeSelf.gotAudio_solveEquations)
                            
                            return animeSelf.gotAudio_findEquations && animeSelf.gotAudio_solveEquations
                        }
                    );
            ...
            //get the audioFile for findEquation before we start the anime
            self.getAudioUrls_findEquations(
                function(subtitles, list_tuple_word_location_length_elapsedTime, filename, pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime){
                    animeSelf.subtitles_findEquations = subtitles; 
                    animeSelf.subtitleIdx__dict_startTime_endTime_findEquations=subtitleIdx__dict_startTime_endTime; 
                ...
                self.textStr__textMeshUUID, 
                self.id__type, 
                animeSelf.language_introduction_findEquations, 
                animeSelf.language_conclusion_findEquations
            );
