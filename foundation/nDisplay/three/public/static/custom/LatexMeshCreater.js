...
class LatexMeshCreator {
    ...
    constructor(scene, renderer, camera, listOfEquations_latexStrs) {
        this.scene = scene; this.renderer = renderer; this.camera = camera;
        this.listOfEquations_latexStrs = listOfEquations_latexStrs;
    }
    ...
    getMeshes(callback) {
      ...
      for (let i=0; i<this.listOfEquations_latexStrs.length; i++) {
          let latexStr = this.listOfEquations_latexStrs[i]; //console.log(latexStr);
          ...
          loader.load(url, (data) => {
            ...
            self.generatedMeshes.push(group);
            callback(i, group);
