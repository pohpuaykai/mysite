...
def bipartiteSearch__dc_twoResistor_parallel(verbose=False):
    # from foundation.automat.core.equation import Equation
    list_equationStrs = [
    '(= I_{R_{0}} I_{DC_{8}})', #0,0#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<?
    '(= I_{R_{0}} I_{R_{3}})', #1,3#<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<?
    '(= (+ (+ (- 0 I_{DC_{8}}) I_{R_{0}}) I_{R_{3}}) 0)', #2,5
    '(= (+ (- 0 V_{R_{0}}) V_{DC_{8}}) 0)', #3,6
    '(= (- V_{DC_{8}} V_{R_{3}}) 0)', #4,9
    '(= (/ V_{R_{0}} I_{R_{0}}) R_{R_{0}})', #5,11
    '(= (/ V_{R_{3}} I_{R_{3}}) R_{R_{3}})', #6,13
    '(= (/ V_{DC_{8}} I_{DC_{8}}) R_{DC_{8}})' #7,15
    ]
    list_variables = [
    'I_{R_{0}}', #0,1
    'I_{DC_{8}}', #1,2
    ...
    equationVariables_bg = {1: [0, 3, 5, 11], 0: [1, 2], 2: [0, 5, 15], 3: [1, 4], 4: [3, 5, 13], 5: [2, 1, 4], 7: [6, 11], 6: [7, 8], 8: [6, 9, 15], 9: [8, 10], 10: [9, 13], 11: [7, 1, 12], 12: [11], 13: [10, 4, 14], 14: [13], 15: [8, 2, 16], 16: [15]}
    vertexId__equationVariableId = {0: 0, 1: 0, 2: 1, 3: 1, 4: 2, 5: 2, 6: 3, 7: 3, 8: 4, 9: 4, 10: 5, 11: 5, 12: 6, 13: 6, 14: 7, 15: 7, 16: 8}
    equationId__vertexId = {0: 0, 1: 3, 2: 5, 3: 6, 4: 9, 5: 11, 6: 13, 7: 15} # during creation of new equation, this must also be updated
    variableId__vertexId = {0: 1, 1: 2, 2: 4, 3: 7, 4: 8, 5: 10, 6: 12, 7: 14, 8: 16} # this need not be updated, because no new variables are created, and so the maximum number of equations creatable, is 2^{numberOfVariables}
    type__list_vertexIds = {'equation': [0, 3, 5, 6, 9, 11, 13, 15], 'variable': [1, 2, 4, 7, 8, 10, 12, 14, 16]} # during creation of new equation, this must also be updated
    equationKey = 'equation'
    variableKey = 'variable'
