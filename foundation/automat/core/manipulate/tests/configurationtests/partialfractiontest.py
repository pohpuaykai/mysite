...
def test__vor0__configTest(verbose=False):
    ...
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (/ (+ (* $1 $0) $2) (* (- $0 $3) (- $0 $4)))
...
def test__hin0__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (+ (/ (- $1 (/ (* (- $2 (* $1 $4)) $4) (- $3 $4))) (- $0 $3)) (/ (/ (* (- $2 (* $4 $1)) $4) (- $3 $4)) (- $0 $4)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__vor1__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (/ (+ (* $1 $0) $2) (^ (- $0 $3) 2))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__hin1__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (+ (/ $1 (- $0 $3)) (/ (+ (* $3 $1) $2) (^ (- $0 $3) 2)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__vor2__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (/ (+ (+ (* $1 (^ $0 2)) (* $2 $0)) $3) (* (* (- $0 $4) (- $0 $5)) (- $0 $6)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__hin2__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (+ (+ (/ (/ (+ (- (* (^ $4 2) $1) (* $4 $2)) $3) (* (- $4 $5) (- $4 $6))) (- $0 $4)) (/ (/ (- (- (* $5 $2) $3) (* (^ $5 2) $1)) (* (- $4 $5) (- $5 $6))) (- $0 $5))) (/ (/ (+ (- $3 (* $6 $2)) (* (^ $6 2) $1)) (* (- $4 $6) (- $5 $6))) (- $0 $6)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__vor3__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (/ (+ (+ (* $1 (^ $0 2)) (* $2 $0)) $3) (* (^ (- $0 $4) 2) (- $0 $5)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__hin3__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (+ (+ (/ (/ (- (- (- (* (^ $4 2) $1) (* (* (* 2 $4) $5) $1)) (* $2 $5)) $3) (^ (- $5 $4) 2)) (- $0 $4)) (/ (/ (+ (+ (^ $4 2) $4) $3) (- $4 $5)) (^ (- $0 $4) 2))) (/ (/ (+ (+ (* (^ $5 2) $1) (* $5 $2)) $3) (^ (- $5 $4) 2)) (- $0 $5)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__vor4__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (/ (+ (+ (* $1 (^ $0 2)) (* $2 $0)) $3) (* (- $0 $4) (+ (+ (^ $0 2) (* $5 $0)) $6)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
...
def test__hin4__configTest(verbose=False):
    ...
    eq0 = Equation(eqs, eqsType)
    ma0 = Partialfraction(direction, idx, verbose=verbose)
    manipulatedSchemeEquation = ma0.apply(eq0) # (+ (/ (/ (+ (+ (* (^ $4 2) $1) (* $4 $2)) $3) (+ (+ (^ $4 2) (* $4 $5)) $6)) (- $0 $4)) (/ (+ (* (/ (+ (- (* (* $4 $5) $1) (* $4 $2)) (- (* $1 $6) $3)) (+ (+ (^ $4 2) (* $4 $5)) $6)) $0) (/ (+ (+ (* (* $4 $1) $6) (* $4 $3)) (- (* $5 $3) (* $6 $2))) (+ (+ (^ $4 2) (* $4 $5)) $6))) (+ (+ (^ $0 2) (* $5 $0)) $6)))
    ast, functionsD, variablesD, primitives, totalNodeCount, startPos__nodeId = Schemeparser(equationStr=manipulatedSchemeEquation)._parse()
    manipulatedAst = ast
