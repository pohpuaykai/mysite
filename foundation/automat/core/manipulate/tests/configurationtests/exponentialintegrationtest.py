...
from foundation.automat.core.manipulate.pattern.exponentialintegration import Exponentialintegration
...
def test__vor0__configTest(verbose=False):
    eqs = '(= y (int (* (^ "e" x) (D x x)) x))' # fill it in
    eqsType = 'scheme'
    #filename = 'exponentialintegration'
    direction = 'vor'
    idx = 0
...
def test__vor1__configTest(verbose=False):
    eqs = '(= y (int (* (^ x x) (+ (* (D x x) (log "e" x)) (* (D x x) (/ x x)))) x))' # fill it in
    eqsType = 'scheme'
    #filename = 'exponentialintegration'
    direction = 'vor'
    idx = 1
