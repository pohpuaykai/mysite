...
solvingSteps = [   {   'hin': {},
    ...
    {   'hin': {   'latex': '(\\frac{1}{R_{DC_{8}}}-\\frac{1}{R_{R_{3}}})R_{R_{0}}=1',
                   ...
                   'scheme': '(= (* (- (/ 1 R_{DC_{8}}) (/ 1 R_{R_{3}})) '
                             'R_{R_{0}}) 1)',
                   'variables': ['R_{R_{0}}', 'R_{R_{3}}', 'R_{DC_{8}}']},
        'hin__subSteps': [],
        'sub': '',
...
runningStepsIdx__branchedStepsIdx = {   '0': [0, 'vor'],
     '1': [   1,
              'vor',
              0,
              'vor__subSteps'],
     ...
     '9': [2, 'hin']}
###
def test_generateSubtitle_solvingStepNoAudio(verbose=False):
    scab = SubtitlesCircuitAnimeBasic()
    languageTwoLetterCode = 'en-US'
    scripts = scab.generateSubtitle_solvingStep(solvingSteps, runningStepsIdx__branchedStepsIdx, languageTwoLetterCode, language_introduction_solvingSteps[languageTwoLetterCode], language_conclusion_solvingSteps[languageTwoLetterCode])
    if verbose:
        pp.pprint(scripts)


def test_generateSubtitle_solvingStepsNoAudio(verbose=False):
    scab = SubtitlesCircuitAnimeBasic()
    scripts = scab.generateSubtitles_solvingSteps(solvingSteps, runningStepsIdx__branchedStepsIdx, language_introduction_solvingSteps, language_conclusion_solvingSteps)
    if verbose:
        pp.pprint(scripts)
...
def test_generateSubtitles_findEquations(verbose=False):
    ...
    ticketNumber = scab.generateAudioFilePaths_findEquations(list_equationNetworkInfoDict, textStr__textMeshUUID, nodeId__type, language_introduction_findEquations, language_conclusion_findEquations, folderpath)
...
def test_generateSubtitles_solvingSteps(verbose=False):
    ...
    # languages__subtitles, languages__subtitleIdx__dict_startTime_endTime, languages__filepath, language__pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime = 
    ticketNumber = scab.generateAudioFilePaths_solvingSteps(solvingSteps, runningStepsIdx__branchedStepsIdx, language_introduction_solvingSteps, language_conclusion_solvingSteps, folderpath)
    response = scab.waitUntilResponseAvailable(ticketNumber)
    if verbose:
        print('***********************')
        # pp.pprint(subtitles)
...
def test_generateSubtitle_findEquation(verbose=False):
    ...
    languageTwoLetterCode = 'en-US'
    # languages__subtitles, languages__subtitleIdx__dict_startTime_endTime, languages__filepath, language__pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime = 
    ticketNumber = scab.generateAudioFilePath_findEquation(list_equationNetworkInfoDict, textStr__textMeshUUID, nodeId__type, languageTwoLetterCode, language_introduction_findEquations[languageTwoLetterCode], language_conclusion_findEquations[languageTwoLetterCode], folderpath)
    response = scab.waitUntilResponseAvailable(ticketNumber)
    if verbose:
        print('***********************')
        # pp.pprint(results)
...
def test_generateSubtitle_solvingStep(verbose=False):
    scab = SubtitlesCircuitAnimeBasic()
    languageTwoLetterCode = 'en-US'
    # languages__subtitles, languages__subtitleIdx__dict_startTime_endTime, languages__filepath, language__pauseIdx__dict_startTime_endTime_listOfWordLocationLengthElapsedTime = 
    ticketNumber = scab.generateAudioFilePath_solvingStep(solvingSteps, runningStepsIdx__branchedStepsIdx, languageTwoLetterCode, language_introduction_solvingSteps[languageTwoLetterCode], language_conclusion_solvingSteps[languageTwoLetterCode], folderpath)
    response = scab.waitUntilResponseAvailable(ticketNumber)
    if verbose:
        print('***********************')
        # pp.pprint(subtitles)
