...
class SubtitlesCircuitAnimeBasic:
    ...
    def translateEquationFinderName(self, defaultEquationFinderDisplayName, languageTwoLetterCode):#<<<<<<<<<<<<move this to a more global folder... there might be other different animations next time, hopefully mechanical stimulations next =)
        return {
    ...
    def translateComponentTypeName(self, componentType, languageTwoLetterCode):
        return {
            "AC_signal_generator":{
    ...
    def translateEliminateVariableToWords(self, subVar, languageTwoLetterCode):
        ...
        }[languageTwoLetterCode][SubtitlesCircuitAnimeBasic.mainSymbol__fullName[main_symbol]]

        translatedComponentType = self.translateComponentTypeName(SubtitlesCircuitAnimeBasic.subscript__fullName[subscript], languageTwoLetterCode) # subscript is the componentType

        # print(languageTwoLetterCode[0:2])
        # import pdb;pdb.set_trace()
        translatedNodeId = self.num_to_word(int(subsubscript), languageTwoLetterCode[0:2])
    ...
    def get__language__templateIdx__templates__findEquations(self):
        return {
            'en-US': {
                0:[
    ...
    def generateSubtitle_findEquation(self, list_equationNetworkInfoDict, textStr__textMeshUUID, nodeId__type, languageTwoLetterCode, introduction, conclusion):
        import random
        # print(nodeId__type)
        # import pdb;pdb.set_trace()
        language__templateIdx__templates = self.get__language__templateIdx__templates__findEquations()
        scripts = []
        scripts.append(introduction)
        for equationFound in list_equationNetworkInfoDict:
            equationFinderDisplayName = self.translateEquationFinderName(equationFound['equationFinderDisplayName'], languageTwoLetterCode) #<<<<<<<<implement this
            ...
            for variableInfoDict in textStr__textMeshUUID[equationFound['equation']]['info']:
                ...
                componentType = self.translateComponentTypeName(nodeId__type[str(nodeId)], languageTwoLetterCode)
    ...
    def generateSubtitles_findEquations(self, list_equationNetworkInfoDict, textStr__textMeshUUID, nodeId__type, introductions, conclusions):
        """
        ...
        for languageTwoLetterCode in languagesTwoLetterCodes:
            ...
            for equationFound in list_equationNetworkInfoDict:
                ...
                for variableInfoDict in textStr__textMeshUUID[equationFound['equation']]['info']:
                    ...
                    # for variableStr in variableInfoDict['variables']:
                    nodeId = equationFound['variableStr__nodeId'][variableInfoDict['variableStr']]
                    componentType = self.translateComponentTypeName(nodeId__type[nodeId], languageTwoLetterCode)
                    #6,8 translate, give another 2 different sentences
                    templates1 = language__templateIdx__templates[languageTwoLetterCode][1]
                    scriptPart1 = Template(templates1[random.randint(0, len(templates1)-1)]).substitute(componentType=componentType, nodeId=str(nodeId))
    ...
    def get__language__templateIdx__templates_solvingSteps(self):#<<<<<<<<<<<<<<<<move this to another file?
        return {
            'en-US': {
                0:[
            ...
            'ja-JP': {
                ...
                1:[
                    "この始める公式には、変数が$subLenあります。",
                    "変数を$subLen含む公式から始めます。",
                    "最初の公式には変数が$subLenあります。"
                ],
                2:[
                    "いま、変数を、$subで消去したい。",
                    "これを削除するのためには、変数の$subを表します。",
                    "変数の$subを消去したいので、表します。"
                ], 
                3:[
                    "次に、この公式での、変数の$subを表します。",
                    "変数の$subを消去するのためには、表します。",
                    "また、変数の$sub を表して消去します。"
                ],
                4:[
                    "それでは、2つの公式を結び付けます。",
                    "今は、2つの公式を組み合わせます。",
                ...
                5:[
                    ...
                    "次に、消去したい変数だけに触れて言及します。"
                ],
                6:[
                    "このステップでは、変数の$subを消去します。",
                    "ここでは、変数の$subを消去します。",
                    "このステップは、変数の$subが消去されます。"
                ],
                7:[
                    "公式を、簡単にしましょう。",
                    "次に、この公式を、簡略化します。",
    ...
    def generateSubtitle_solvingStep(self, solvingSteps, runningStepsIdx__branchedStepsIdx, languageTwoLetterCode, introduction, conclusion):
        import random
        scripts = []
        language__templateIdx__templates = self.get__language__templateIdx__templates_solvingSteps()
        templateIdx__templates =language__templateIdx__templates[languageTwoLetterCode]
        def addScriptToList(templateNum, kwargs, joinToPrevious=False):
            ...
            if joinToPrevious:
                scripts[-1] += os.linesep+ scriptPart0
            else:
                scripts.append(scriptPart0)#insert pause?
        scripts.append(introduction)
        #first long script
        addScriptToList(0, {}, joinToPrevious=True)#in this script, the first sentence is very much like an introduction, so we will treat this sentence as an extended_variable_introduction
        firstStep = solvingSteps[0]['vor']
        # print(firstStep)
        addScriptToList(1, {'subLen':len(firstStep['variables'])}, joinToPrevious=False)
        translate_variable_to_words = self.translateEliminateVariableToWords(solvingSteps[1]['sub'], languageTwoLetterCode)
        addScriptToList(2, {'sub':translate_variable_to_words}, joinToPrevious=True)
        addScriptToList(3, {'sub':translate_variable_to_words}, joinToPrevious=True)
        addScriptToList(4, {}, joinToPrevious=True)
        addScriptToList(5, {}, joinToPrevious=True)
        #linearise the solvingSteps
        for runningStepsIdx, branchedStepsIdx in sorted(runningStepsIdx__branchedStepsIdx.items(), key=lambda t: int(t[0])):
            if runningStepsIdx == '0': #skip the first one...
                continue
            #
            # print('len(solvingSteps)', len(solvingSteps), 'branchedStepsIdx', branchedStepsIdx, 'branchedStepsIdx[0]:')
            # import pdb;pdb.set_trace()
            pointer = solvingSteps[branchedStepsIdx[0]]

            if len(branchedStepsIdx) > 3: #is a substep, for now, all substeps are 'stepType':'simplification'
                # pointer = pointer[branchedStepsIdx[-1]][branchedStepsIdx[-2]] # this gets the exact step that was executed, for now we do not need it
                # print('substep: ', 'sub' not in pointer)
                addScriptToList(7, {})
            else:
                ...
                # print(pointer['sub'] == '')
                # pointer = pointer[branchedStepsIdx[1]] # this gets the exact step that was executed, for now we do not need it
            # import pdb;pdb.set_trace()
            #
            # print(pointer)
            # import pdb;pdb.set_trace()
    ...
    def generateSubtitles_solvingSteps(self, solvingSteps, runningStepsIdx__branchedStepsIdx, introductions, conclusions):#maybe adding some conjunctions might smoothen the narration?<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        """
        ...
        languagesTwoLetterCodes = list(introductions.keys())
        languages__scripts = dict(map(lambda languageCode: (languageCode, []), languagesTwoLetterCodes))
        def addScriptToList(languageTwoLetterCode, templateNum, kwargs, joinToPrevious=False):
            templates0 = language__templateIdx__templates[languageTwoLetterCode][templateNum]
            scriptPart0 = Template(templates0[random.randint(0, len(templates0)-1)]).substitute(**kwargs)
            if joinToPrevious:
                languages__scripts[languageTwoLetterCode][-1] += os.linesep+ scriptPart0
            else:
                languages__scripts[languageTwoLetterCode].append(scriptPart0)
            # languages__scripts[languageTwoLetterCode].append(scriptPart0)#insert pause?
        ...
        for languageTwoLetterCode in languagesTwoLetterCodes:
            languages__scripts[languageTwoLetterCode].append(introductions[languageTwoLetterCode])
            addScriptToList(languageTwoLetterCode, 0, {}, joinToPrevious=False)
            firstStep = solvingSteps[0]['vor']
            addScriptToList(languageTwoLetterCode, 1, {'subLen':len(firstStep['variables'])}, joinToPrevious=True)
            translate_variable_to_words = self.translateEliminateVariableToWords(solvingSteps[1]['sub'], languageTwoLetterCode)
            addScriptToList(languageTwoLetterCode, 2, {'sub':translate_variable_to_words}, joinToPrevious=True)
            #insert pause<<<<<<<<<<<<<<<<<<<<<
            addScriptToList(languageTwoLetterCode, 3, {'sub':translate_variable_to_words}, joinToPrevious=True)
            #insert pause<<<<<<<<<<<<<<<<<<<<<
            addScriptToList(languageTwoLetterCode, 4, {}, joinToPrevious=True)
            addScriptToList(languageTwoLetterCode, 5, {}, joinToPrevious=True)
            for runningStepsIdx, branchedStepsIdx in sorted(runningStepsIdx__branchedStepsIdx.items(), key=lambda t: t[0]):
                if runningStepsIdx == '0': #skip the first one...
                    continue
                pointer = solvingSteps[branchedStepsIdx[0]]
                if len(branchedStepsIdx) > 3: #is a substep, for now, all substeps are 'stepType':'simplification'
    ...
    def getResponseIfAvailable(self, ticketNumber):
        return SubtitlesCircuitAnimeBasic.aft.getReturnOfDatabaseTicket(ticketNumber)

    def waitUntilResponseAvailable(self, ticketNumber):
        response = self.getResponseIfAvailable(ticketNumber)
