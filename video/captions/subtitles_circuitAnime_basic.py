...
def generateSubtitles_findEquations(list_equationNetworkInfoDict, textStr__textMeshUUID, nodeId__type, introductions, conclusions):
    ...
    """
    import random
    language__templateIdx__templates = {
        'en-US': {
            0:[
...
def generateSubtitles_solvingSteps(solvingSteps, introductions, conclusions):#maybe adding some conjunctions might smoothen the narration?<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    language__templateIdx__templates = {
        ...
        'zh-CN': {
            0:[
                "我们依此术解答：先取一个方程，将需要消去的变数挪为主项，再取另一个方程，将需要消去的变量也挪为主项，然后把两个方程连接起来，消去主项。重复这个过程，直到消去所有不需要的变数。",
                "先取一个方程，解出我们想要消除的变数，然后取另一个方程，解出同一个变数，最后将两个方程连接起来，最终消去该变数。重复此过程，直到消去所有不需要的变数。",
                "我们将逐个解方程，将需要消去的变数作为每个方程的主项，然后每一步将两个方程连接起来，消去该变数。最终我们能得到想要的公式。"
            ],
            1:[
                "这开头方程有$subLen个变数",
                "我们从有$subLen个变数的方程开始",
            ...
            5:[
                "然后我们不断重复，所以从现在开始，我们只会提到我们想要消除的变数",
        ...
        'de-DE': {
            0:[
                "Unsere übliche Methode besteht darin, eine Gleichung zu nehmen, die Variable, die wir eliminieren möchten, zum Subjekt zu machen, eine andere Gleichung zu nehmen und darin die Variable zu machen, die wir eliminieren möchten, dann verbinden wir die beiden Gleichungen, um diese Variable zu eliminieren. Wir wiederholen dies, bis wir alle Variablen eliminiert haben, die wir nicht wollen.",
                "Eine einfache Methode wäre, eine Gleichung zu nehmen und nach der zu eliminierenden Variable aufzulösen. Dann eine andere Gleichung zu nehmen und nach derselben Variable aufzulösen. Anschließend verbinden wir die beiden Gleichungen miteinander und eliminieren schließlich diese Variable. Und wiederholen diesen Vorgang, bis wir alle unerwünschten Variablen los sind.",
                "Wir werden eine Gleichung nach der anderen durchgehen, die Variable, die wir eliminieren möchten, zum Subjekt jeder Gleichung machen und bei jedem Schritt die beiden Gleichungen verbinden, um die Variable zu eliminieren. Hoffentlich erhalten wir schließlich die gewünschte Formel."
            ],
            1:[
                "Wir haben eine Ausgangsgleichung mit $subLen-Variablen",
                "Wir beginnen mit dieser Gleichung mit $subLen-Variablen",
                "Unsere Ausgangsgleichung hat $subLen-Variablen"
            ],
            2:[
                "Wir möchten die Variable eliminieren: $sub",
            ...
            5:[
                "Und dann wiederholen wir es weiter, also werden wir von nun an nur noch die Variable erwähnen, die wir eliminieren wollen",
                "Und jetzt wiederholen wir einfach weiter und eliminieren die Variablen, die wir nicht wollen",
                "Von nun an werden wir einfach die Variablen eliminieren, die wir nicht wollen"
            ],
            6:[
