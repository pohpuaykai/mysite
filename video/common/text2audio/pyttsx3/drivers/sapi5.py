...
#from video.common.audiofromtext.pyttsx3.voice import Voice
...
class SAPI5Driver:
    ...
    def say(self, text) -> None:
        #self._proxy.setBusy(True) # this will pump the queue
        self._proxy._busy = True
        print('start started-utterance notification<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        self._proxy.notify("started-utterance")
        print('end started-utterance notification<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        self._speaking = True
        print('sapi5.py, self._speaking (should be true)', self._speaking)
    ...
    def startLoop(self) -> None:
        ...
        while self._looping:
            ...
            time.sleep(0.05) # needs to be fast to get the realtime for subtitling
            #print('pythoncom.PumpWaitingMessages()')
            #print('self._advise', self._advise, dir(self._advise))
            #time.sleep(3) # wait for 3 second

    def endLoop(self) -> None:
        self._looping = False
        print('sapi5 called endLoop:', self._looping, '<<<<<<<<<<<<<<<<<<<<<>>>>>>>>>>>>>>>>>>>>>>>>>>')
...
class SAPI5DriverEventSink:
    ...
    def _ISpeechVoiceEvents_EndStream(self, stream_number, stream_position) -> None:
        d = self._driver
        print('$$$$$$$ calling EndStream, d._speaking: ', d._speaking)
        if d._speaking:
            d._proxy.notify("finished-utterance", completed=not d._stopping)
            print('$$$$$$ notified finished-utterance')
        d._speaking = False
        d._stopping = False
        d._looping = False
        d.endLoop()
        d._proxy.setBusy(False)
        #d.endLoop()  # hangs if you dont have this, but we do our own recursion, so we will endLoop ourself
    ...
    def _ISpeechVoiceEvents_Word(self, stream_number, stream_position, char, length) -> None:
        ...
        self._driver._proxy.notify("started-word", word=current_word, location=char, length=length) # leave the name for other uses like a callback function
